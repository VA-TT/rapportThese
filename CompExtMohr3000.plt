...
plot y_tangent_common1(x) with lines lc rgb colors[1] title sprintf('$I = 10^{-4}: \varphi = %.2f^{\circ}, c = %.2f$ kPa', phi_common_degree1, c_common1),\
     y_tangent_common2(x) with lines lc rgb colors[2] title sprintf('$I = 10^{-3}: \varphi = %.2f^{\circ}, c = %.2f$ kPa', phi_common_degree2, c_common2),\
     y_tangent_common3(x) with lines lc rgb colors[3] title sprintf('$I = 10^{-2}: \varphi = %.2f^{\circ}, c = %.2f$ kPa', phi_common_degree3, c_common3),\
...
array Ivals[8] = [1e-4, 1e-3, 1e-2, 2e-2, 4e-2, 6e-2, 8e-2, 1e-1]
array muvals[8]
muvals[1] = m_common1
muvals[2] = m_common2
muvals[3] = m_common3
...
array titles[8] = ["$I = 10^{-4}$", "$I = 10^{-3}$", "$I = 10^{-2}$", "$I = 2 \\times 10^{-2}$", \
                   "$I = 4 \\times 10^{-2}$", "$I = 6 \\times 10^{-2}$", "$I = 8 \\times 10^{-2}$", "$I = 10^{-1}$"]

set xtics (titles[1] Ivals[1], titles[2] Ivals[2], titles[3] Ivals[3], titles[8] Ivals[8])

# write the arrays to a temporary data file for fitting and plotting
tmpfile = 'Mu_from_mcommon.dat'
set print tmpfile
do for [i=1:8] { print Ivals[i], muvals[i] }
set print
